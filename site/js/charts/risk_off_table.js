...
// The fund column does not depend on it: its duration belongs to a 3-10yr fund
// priced off the 7yr yield, and the header says so.
//
// Payload: { tenors, threshold_pct, fund: {duration_years, tenor},
// rows: [{id, label, sessions, days, avg_sp_fall_pct, fund_move_pct,
// correlation: {tenor: r}, by_tenor: {tenor: {avg_change_bp, share_fell_pct}}}] }.
...
export function render(el, ctx) {
    ...
    function draw() {
        ...
        const head = `<tr><th>Period</th><th class="num">Bad days</th><th class="num">Avg S&amp;P fall</th>`
            ...
            + `<th class="num">Est. fund price move (${data.fund.tenor}, fixed)</th>`
            + `<th class="num">Stock–bond correlation (${active})</th></tr>`;
        const body = data.rows.map(r => {
            const t = r.by_tenor[active];
            const c = r.correlation[active];
            const corrText = c === null || c === undefined ? '—' : c.toFixed(2);
            const corrColor = c === null || c === undefined ? null : (c >= 0 ? theme.up : theme.down);
            if (!t) {
                return `<tr><th scope="row">${r.label}</th>${num(r.days)}${num('—')}${num('—')}${num('—')}${num('—')}`
                    + `${num(corrText, corrColor)}</tr>`;
            }
        ...
        const foot = [
            ...
            `Est. fund price move = −${data.fund.duration_years} × the average change in the ${data.fund.tenor} yield (an intermediate Treasury fund's duration). It does not change with the yield picked above, because that duration belongs to a fund that tracks the ${data.fund.tenor} point.`,
            `Correlation: S&P daily return against the daily change in the ${active} yield, all sessions in the period. Positive = bond prices rise on bad stock days (hedge working); negative = they fall.`,
...
export function csv(ctx) {
    ...
    const header = ['period', 'sessions', 'bad_days', 'avg_sp_fall_pct', 'tenor', 'avg_yield_change_bp',
        'share_yield_fell_pct', 'fund_move_pct', 'correlation'].join(',');
    const lines = data.rows.flatMap(r => Object.keys(r.by_tenor).map(t => [
        ...
        r.by_tenor[t].share_fell_pct, r.fund_move_pct, r.correlation[t] === null ? '' : r.correlation[t],
    ].join(',')));
