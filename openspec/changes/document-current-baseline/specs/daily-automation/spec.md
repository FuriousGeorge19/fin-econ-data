...
- **THEN** the job checks out the repo, sets up Python 3.12, installs dependencies, and
  runs `fetch_treasury.py`, `fetch_sp500_pe.py`, and `fetch_yield_curve.py`
...
- **THEN** the workflow copies `dgs10.json`, `sp500_pe.json`, and `yield_curve.json` into
...
GitHub Actions secret. The Treasury and yield-curve steps SHALL receive it; the Pages
deploy SHALL use the automatic `GITHUB_TOKEN`.

#### Scenario: Secret injected into fetch steps

- **WHEN** the Treasury and yield-curve fetch steps run in CI
- **THEN** they receive `FRED_API_KEY` from `secrets.FRED_API_KEY` in their environment
